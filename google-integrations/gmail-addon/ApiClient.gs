/**
 * ApiClient.gs — HTTP client for the FluxDesk REST API (Gmail Add-on)
 *
 * Mirrors the Chat bot client with identical error handling.
 ...
 *
 * @param {string} toolId  - Tool identifier (e.g. "email-intent-decoder", "standup")
 * @param {Object} inputs  - Tool-specific input fields
...
function callTool(toolId, inputs, token) {
  ...
  if (code === 401) {
    throw new Error('Your FluxDesk token is invalid or expired. Update it in the sidebar settings.');
  }
  if (code === 404) {
