/**
 * Config.gs — FluxDesk Gmail Add-on configuration helpers
 *
 ...
 * Falls back to DEFAULT_API_URL if not set. Trailing slash always stripped.
 */
function getApiUrl() {
 ...
 * Should only be called once during deployment setup.
 *
...
/**
 * Returns the JWT access token for the current user, or null if not set.
 */
...
/**
 * Persists the user's JWT access token.
 *
...
/**
 * Clears the stored token for the current user.
 */
