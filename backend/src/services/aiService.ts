...
// Simple reversible encryption for API keys (use KMS in production)
...
async function selectProvider(
  ...
  // Fall back to user's most-used provider from memory
...
export async function callAI(options: AICallOptions): Promise<{ text: string; provider: AIProvider }> {
  ...
  try {
    ...
    if (provider === 'CLAUDE') {
      ...
      text = resp.content.filter(b => b.type === 'text').map(b => (b as any).text).join('');
    ...
    // Provide clear provider-specific error messages
    if (err.status === 401 || err.message?.includes('invalid') || err.message?.includes('API key')) {
      throw Object.assign(
